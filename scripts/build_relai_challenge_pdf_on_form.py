...
def make_table(data: list[list[str]], widths: list[float], font_size: float = 6.7) -> Table:
...
def overlay_pdf(story: list, *, clear: list[tuple[float, float, float, float]], heading: str | None = None) -> BytesIO:
    packet = BytesIO()
    ...
    if heading:
        c.setFillColor(colors.white)
        c.rect(70, 755, 205, 22, stroke=0, fill=1)
        c.setFillColor(colors.black)
        c.setFont("KR-Bold", 12)
        c.drawString(72, 762, heading)
    frame = Frame(63, 68, 470, 660, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
...
def overlay_pdf_page2(story: list) -> BytesIO:
    packet = BytesIO()
    c = canvas.Canvas(packet, pagesize=A4)
    # Keep the blue title, team table, section label, and outer border.
    c.setFillColor(colors.white)
    c.rect(58, 72, 480, 565, stroke=0, fill=1)
    frame = Frame(63, 78, 470, 548, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
    frame.addFromList(story, c)
    c.save()
    packet.seek(0)
    return packet
...
def add_overlay(base_page, overlay_stream: BytesIO):
    base_page = deepcopy(base_page)
...
def build() -> None:
    ...
    body = ParagraphStyle("body", fontName="KR", fontSize=7.6, leading=10.1, wordWrap="CJK", spaceAfter=4)
    small = ParagraphStyle("small", fontName="KR", fontSize=6.8, leading=8.6, wordWrap="CJK", spaceAfter=3)
    sub = ParagraphStyle("sub", fontName="KR-Bold", fontSize=8.5, leading=11, wordWrap="CJK", spaceAfter=4)
    cap = ParagraphStyle("cap", fontName="KR", fontSize=6.6, leading=8, alignment=TA_CENTER, wordWrap="CJK")
    ...
    # Proposal page 1: latest form page 2 background.
    s1 = [
    ...
    pages.append(add_overlay(reader.pages[1], overlay_pdf_page2(s1)))

    # Proposal page 2: latest form page 3 background.
    s2 = [
    ...
    pages.append(add_overlay(reader.pages[2], overlay_pdf(s2, clear=[(58, 75, 480, 700)])))

    # Proposal page 3: latest form page 5 background.
    s3 = [
        para(
        ...
        para(
            "가장 큰 차별점은 직군 간 언어 변환입니다. 기획자의 요구사항은 개발자가 판단할 수 있는 구현 범위, API 영향, 예외 처리, 테스트 조건으로 변환됩니다. 반대로 개발자의 기술 제약과 리스크는 기획자와 승인권자가 이해할 수 있는 사용자 영향, 일정 영향, 정책 선택지, 승인 필요 항목으로 변환됩니다.",
            body,
        ),
        make_table(
            [
        ...
        Spacer(1, 4),
        para(
            "RelAi는 기존 협업 도구를 대체하기보다, 그 위에 올라가는 AI 정렬 레이어로 작동합니다. Mantis, Confluence, Jira, Notion, Slack, Teams 등에 흩어진 요구사항, 이슈, 회의 기록, 채팅을 AI가 하나의 프로젝트 맥락으로 연결하고, 이를 결정·질문·승인·영향·액션 단위로 재구성합니다.",
            body,
        ),
        para(
            "또한 변경 영향 맵을 통해 요구사항 변경이 화면, API, 데이터, 테스트, 일정, 담당자, 승인권자에게 어떻게 확산되는지 시각화합니다.",
            body,
        ),
        image_flow(img2, 142 * mm, 45 * mm),
        para("그림 2. 변경 요청이 화면, API, 데이터, 테스트, 일정, 담당자, 승인권자로 확산되는 구조", cap),
    ]
    pages.append(add_overlay(reader.pages[4], overlay_pdf(s3, clear=[(58, 75, 480, 680)])))

    # Proposal page 4: latest form page 6 background.
    s4 = [
        para(
    ...
    pages.append(add_overlay(reader.pages[5], overlay_pdf(s4, clear=[(58, 75, 480, 680)])))

    # Proposal page 5: use page 6 form and replace heading with section 4.
    s5 = [
        para(
    ...
    pages.append(add_overlay(reader.pages[5], overlay_pdf(s5, clear=[(58, 75, 480, 680)], heading="4. 사회적/경제적 파급 효과")))

    # Proposal only.
    proposal_writer = PdfWriter()
    for page in pages:
        proposal_writer.add_page(page)
    with PROPOSAL_ONLY.open("wb") as f:
        proposal_writer.write(f)

    # Full final: latest application page + exact-form proposal + latest pledge/privacy pages.
    writer = PdfWriter()
